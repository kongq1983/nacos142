...
public class ConfigCacheService {
    ...
    public static boolean remove(String dataId, String group, String tenant) {
        ...
        try {
            if (!PropertyUtil.isDirectRead()) {
                DiskUtil.removeConfigInfo(dataId, group, tenant); // 删除文件
            }
    ...
    public static boolean removeBeta(String dataId, String group, String tenant) {
        ...
        try {
            if (!PropertyUtil.isDirectRead()) { // standalone 模式下 比如使用外部数据源
                DiskUtil.removeConfigInfo4Beta(dataId, group, tenant);  // 删除本地beta文件
            }
