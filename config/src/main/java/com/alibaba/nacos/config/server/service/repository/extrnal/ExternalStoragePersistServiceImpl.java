...
public class ExternalStoragePersistServiceImpl implements PersistService {
    ...
    public void addConfigInfo(final String srcIp, final String srcUser, final ConfigInfo configInfo,
        ...
        boolean result = tjt.execute(status -> {
            try {
                long configId = addConfigInfoAtomic(-1, srcIp, srcUser, configInfo, time, configAdvanceInfo); // 持久化config_info
                String configTags = configAdvanceInfo == null ? null : (String) configAdvanceInfo.get("config_tags");
                addConfigTagsRelation(configId, configTags, configInfo.getDataId(), configInfo.getGroup(),
                        configInfo.getTenant()); // config_tags_relation 如果有tag 查关联表
                insertConfigHistoryAtomic(0, configInfo, srcIp, srcUser, time, "I"); // 插历史表
